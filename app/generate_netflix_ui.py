...
<body>
  ...
  <main id="moviesView" class="view-container" style="display: none;">
    <div class="view-header">
      ...
      <div class="filter-chips">
        ...
        <button class="chip" data-filter="franchise" onclick="setMovieFilter('franchise')">Franquias & Sagas</button>
        <button class="chip" data-filter="standalone" onclick="setMovieFilter('standalone')">Filmes Avulsos</button>
        <button class="chip" data-filter="4k" onclick="setMovieFilter('4k')">4K Ultra HD</button>
        <button class="chip" data-filter="1080p" onclick="setMovieFilter('1080p')">1080p Full HD</button>
        <button class="chip" data-filter="subtitled" onclick="setMovieFilter('subtitled')">Com Legenda (.srt)</button>
      </div>
    </div>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', async () => {
      ...
      // Se rodando via HTTP, consulta capacidades de VLC e FFmpeg
    ...
    function renderHomeCarousels() {
      ...
      const recent = [...state.movies].reverse().slice(0, 14);
    ...
    function renderMoviesGrid() {
      ...
      if (state.movieFilter === 'franchise') {
        filtered = filtered.filter(m => m.franchise);
      } else if (state.movieFilter === 'standalone') {
        filtered = filtered.filter(m => !m.franchise);
      } else if (state.movieFilter === '4k') {
        filtered = filtered.filter(m => m.resolution.includes('4K'));
      } else if (state.movieFilter === '1080p') {
        filtered = filtered.filter(m => m.resolution.includes('1080p'));
    ...
    /* PLAYER & REPRODUÇÃO */
    function openPlayerById(movieId) {
      const movie = state.movies.find(m => m.id === movieId);
      if (movie) openPlayer(movie);
    }

    function playEpisode(seriesId, seasonIdx, epNum) {
      const series = state.series.find(s => s.id === seriesId);
      if (!series) return;
      const season = series.seasons[seasonIdx];
    ...
    function playNextEpisode() {
      if (!state.currentEpisodeSeries) return;
      const { series, seasonIdx, epNum } = state.currentEpisodeSeries;
      const season = series.seasons[seasonIdx];
      const nextEp = season.episodes.find(e => e.episodeNumber === epNum + 1);
      if (nextEp) {
        showToast(`Iniciando Episódio ${nextEp.episodeNumber}...`);
        playEpisode(series.id, seasonIdx, nextEp.episodeNumber);
      } else {
    ...
    function openPlayer(media, isEpisode = false) {
      state.currentPlaying = media;
      const video = document.getElementById('mainVideo');

      document.getElementById('playerTitle').innerText = isEpisode ? `${state.currentEpisodeSeries.series.title} • ${media.title}` : media.title;
      document.getElementById('playerSubInfo').innerText = `${media.resolution} • ${media.format} ${media.isExtended ? '• Versão Estendida' : ''}`;
    ...
    function setupSeekDrag() {
      ...
      video.addEventListener('progress', () => {
        if (video.buffered.length > 0 && video.duration) {
          ...
          document.getElementById('seekBuffered').style.width = buf + '%';
        }
      });
    }
    ...
    function showHelpModal() {
      alert("CineLocal Home Theater:\n\n• Início: Destaques, carrosséis de filmes, séries e ordem cronológica da Marvel.\n• Filmes: Grade completa de longas com filtros rápidos.\n• Séries: The Bear e coleções de episódios.\n• Atalhos: Espaço (Pause), Setas (Pular 10s), F (Tela Cheia), V (Abrir no VLC Portable 5.1).");
    }
  </script>
</body>
</html>
