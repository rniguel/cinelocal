  ...
  <style>
    ...
    * {
    ...
    .player-ambient-glow {
      ...
      opacity: 0;
      border-radius: 20px;
      transition: opacity 0.5s ease, background 0.4s ease, box-shadow 0.4s ease;
    }
    .player-ambient-glow.active {
      opacity: 0.8;
    }
...
<body>
  ...
  <div id="playerModal" class="player-modal">
    <div id="playerContainer" class="player-container">
      ...
      <div class="video-viewport" id="videoViewport" onclick="handleViewportClick(event)">
        <div id="playerAmbientGlow" class="player-ambient-glow active"></div>
        <canvas id="ambilightCanvas" width="32" height="18" style="display: none;"></canvas>
  ...
  <script>
    ...
    function openPlayer(media, isEpisode = false, startTime = 0) {
      ...
      // Ambilight
      const glow = document.getElementById('playerAmbientGlow');
      if (glow) glow.classList.toggle('active', state.ambilightEnabled);
    ...
    function startAmbilightLoop() {
      ...
      const canvas = document.getElementById('ambilightCanvas');
      const glow = document.getElementById('playerAmbientGlow');
      const video = document.getElementById('mainVideo');
      if (!canvas || !glow || !video) return;
      ...
      ambilightInterval = setInterval(() => {
        ...
        try {
          ...
          if (count > 0) {
            ...
            glow.style.boxShadow = `0 0 110px 35px rgba(${r}, ${g}, ${b}, 0.8)`;
            glow.style.background = `radial-gradient(circle, rgba(${r}, ${g}, ${b}, 0.55) 0%, rgba(${r}, ${g}, ${b}, 0.15) 60%, transparent 100%)`;
          }
        } catch (e) {}
      }, 300);
    }
    ...
    function renderStatsData() {
      ...
      if (window.MCU_CANONICAL_TIMELINE) {
        window.MCU_CANONICAL_TIMELINE.forEach(m => {
          if (watched[m.id] || isMcuTitleWatched(m)) mcuWatched++;
        });
      }
