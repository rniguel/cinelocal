...
<body>
      ...
      <div class="header-right">
        <div class="search-container">
          ...
          <input 
            ...
            class="search-input" 
            placeholder="Buscar títulos, franquias..." 
        ...
        <button class="btn-header-action" onclick="openQrConnectModal()" title="Conectar Smart TV ou Celular">
  ...
  <main id="moviesView" class="view-container" style="display: none;">
    <div class="view-header">
      ...
      <div class="filter-chips">
        ...
        <select id="genreSelectFilter" onchange="setMovieGenreFilter(this.value)" style="background: rgba(255,255,255,0.06); border: 1px solid var(--border); color: #fff; border-radius: 20px; padding: 6px 12px; font-size: 13px; font-weight: 600; cursor: pointer;">
          <option value="">Gênero: Todos</option>
        </select>
        <select id="yearSelectFilter" onchange="setMovieYearFilter(this.value)" style="background: rgba(255,255,255,0.06); border: 1px solid var(--border); color: #fff; border-radius: 20px; padding: 6px 12px; font-size: 13px; font-weight: 600; cursor: pointer;">
        ...
        <div class="controls-row">
          ...
          <div class="controls-right">
            ...
            <div class="dropdown">
              ...
              <div id="subtitlesMenu" class="dropdown-menu">
  ...
  <!-- MODAL CONECTAR TV / CELULAR (QR CODE) -->
  <div id="qrModal" class="series-modal" onclick="closeQrModalOnOut(event)">
    <div class="series-modal-box qr-modal-box">
      ...
      <h2 style="font-size: 1.4rem; color: #fff; margin-bottom: 6px;">Conectar Smart TV ou Celular</h2>
      ...
      <p style="color: var(--text-muted); font-size: 0.8rem; margin: 0;">
        Ou digite o endereço acima no navegador web da sua Smart TV.
      </p>
    </div>
  </div>
  ...
  <script>
    ...
    function renderMoviesGrid() {
      ...
      if (state.movieFilter === 'unwatched') {
    ...
    function renderSeriesGrid() {
      ...
      grid.innerHTML = state.series.map(s => createCardHtml(s, false)).join('');
      document.getElementById('seriesCountLabel').innerText = `${state.series.length} séries disponíveis`;
    }
    ...
    /* MODAL CONECTAR SMART TV OU CELULAR (QR CODE) */
    function openQrConnectModal() {
      const modal = document.getElementById('qrModal');
      if (!modal) return;

      const displayUrl = state.localUrl || `http://${window.location.hostname || 'localhost'}:8000/`;
      const urlEl = document.getElementById('qrLocalUrlDisplay');
      if (urlEl) urlEl.innerText = displayUrl;

      const qrWrapper = document.getElementById('qrSvgWrapper');
      if (qrWrapper) {
        const qrApiUrl = `https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent(displayUrl)}&bgcolor=13-17-22&color=ffffff&margin=10`;
        qrWrapper.innerHTML = `
          <img src="${qrApiUrl}" alt="QR Code CineLocal" style="width: 220px; height: 220px; border-radius: 12px; box-shadow: 0 4px 16px rgba(0,0,0,0.6);" onerror="this.onerror=null; this.parentElement.innerHTML='<div style=\\'padding: 24px; color: var(--text-muted); font-size: 0.9rem; text-align: center;\\'>📡 Conecte seu dispositivo na mesma rede Wi-Fi e digite o endereço abaixo no navegador.</div>';">
        `;
      }
      ...
      window.addEventListener('keydown', e => {
        ...
        if (isPlayerOpen) {
          if (e.key === ' ' || e.key.toLowerCase() === 'k') {
            ...
            e.preventDefault();
            state.subSyncOffset = Math.round((state.subSyncOffset - 0.1) * 10) / 10;
            showToast(`Sincronia legenda: ${state.subSyncOffset > 0 ? '+' : ''}${state.subSyncOffset}s`);
          ...
          // Fora do player: Smart TV D-Pad e Enter
