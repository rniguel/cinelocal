...
<body>
  ...
  <script>
    ...
    function getEffectiveDuration() {
      const video = document.getElementById('mainVideo');
      if (state.mediaTotalDuration && state.mediaTotalDuration > 0) {
    ...
    function selectAudioTrack(media, audioIndex) {
      ...
      if (audioIndex === 1) {
        ...
        video.src = encodeURI(media.videoPath);
        video.onloadedmetadata = () => {
          if (currentEffective > 0) video.currentTime = currentEffective;
          video.play().catch(() => {});
        };
        video.load();
      } else {
    ...
    function skipTime(seconds) {
      ...
      const target = Math.max(0, Math.min(dur || Infinity, curr + seconds));
